...
def generate_readme(source_stats: dict, old_stats: dict, out_dir: Path):
    ...
    source_table = []
    source_table.append("| Name | Block | White | Total Lines | Status |")
    source_table.append("|------|-------|-------|-------------|--------|")
    ...
    for name, stats in sorted(source_stats.items()):
        if isinstance(stats, dict):
            ...
            status = "OK" if not error_msg else f"ERROR: {error_msg[:20]}"
            
            source_table.append(f"| {name} | {block_count:,} | {white_count:,} | {total_lines:,} | {status} |")
    
    readme_content = f"""# ADH-AD Ad Domain Rules
...
- Block domains: {total_block:,}
- White domains: {total_white:,}
- Upstream sources: {len(source_stats)}
    ...
    try:
        ...
        try:
            ...
            if not (OUT / "adguardhome.txt").exists():
                write_file(OUT / "adguardhome.txt", "! Error occurred during build\n", "error-file")
            if not (OUT / "dnsmasq.conf").exists():
                write_file(OUT / "dnsmasq.conf", "# Error occurred during build\n", "error-file")
            if not (OUT / "clash.yaml").exists():
                write_file(OUT / "clash.yaml", "payload:\n  # Error occurred during build\n", "error-file")
            if not (OUT / "README.md").exists():
                write_file(OUT / "README.md", f"# Error occurred\n\n{e}\n", "error-file")
        except:
            pass
        sys.exit(1)
